...
def cal(distance, yaw):
    ...
    y = math.cos(math.radians(180 - yaw))
...
# 골 포인트 이동 (정원혁)
def move_to_goal_point(pallet_path,forklift_path):
    # acquire stage
    stage = omni.usd.get_context().get_stage()
    # resolve prim paths for spawning and cloning
    pallet_prim_paths = sim_utils.find_matching_prim_paths(pallet_path)
    forklift_prim_paths = sim_utils.find_matching_prim_paths(forklift_path)
    # manually clone prims if the source prim path is a regex expression
    with Sdf.ChangeBlock():
        for pallet_prim_path,forklift_prim_path in zip(pallet_prim_paths,forklift_prim_paths):
            # spawn single instance
            pallet_prim_spec = Sdf.CreatePrimInLayer(stage.GetRootLayer(), pallet_prim_path)
            forklift_prim_spec = Sdf.CreatePrimInLayer(stage.GetRootLayer(), forklift_prim_path)
            
            # DO YOUR OWN OTHER KIND OF RANDOMIZATION HERE!
            forklift_translate_spec = forklift_prim_spec.GetAttributeAtPath(forklift_prim_path + ".xformOp:translate")
            forklift_orient_spec = forklift_prim_spec.GetAttributeAtPath(forklift_prim_path + ".xformOp:orient")

            forklift_prim = stage.GetPrimAtPath(forklift_prim_path)
            pallet_prim = stage.GetPrimAtPath(pallet_prim_path)
            
            forklift_translate = forklift_prim.GetAttribute("xformOp:translate")
            pallet_orient = pallet_prim.GetAttribute("xformOp:orient")
            pallet_translate = pallet_prim.GetAttribute("xformOp:translate")
            
            
            #쿼터니언 값을 xyz축으로 변환 후, yaw값 추출
            yaw = pallet_orient.Get()
            print(yaw)
            yaw = quat2yaw(yaw)
            print(yaw)
                 
            goal_point = decide(
                s0 = forklift_translate.Get()[:-1],
                s1 = pallet_translate.Get()[:-1],
                distance = 1,
                yaw = yaw)
            goal_point = (*goal_point,0)
            
            if forklift_orient_spec is None:
                forklift_orient_spec = Sdf.AttributeSpec(forklift_prim_spec, "xformOp:orient", Sdf.ValueTypeNames.Quatd)
            forklift_orient_spec.default = pallet_orient.Get()
            if forklift_translate_spec is None:
                forklift_translate_spec = Sdf.AttributeSpec(forklift_prim_spec, "xformOp:translate", Sdf.ValueTypeNames.Double3)
            forklift_translate_spec.default = goal_point
# (여기까지 수정함)
...
class ForkliftSceneCfg(InteractiveSceneCfg):
    ...
    pallet = AssetBaseCfg(
        ...
        init_state=AssetBaseCfg.InitialStateCfg(pos=(0.0,0.0,0.0),rot=(0.0, 0.0, 0.7071, 0.7071)), # axis-x rotation 90 degree ,rot=(0.7071, 0.0, 0.0, 0.7071)
...
# (여기서부터)    
    ...
    move_to_goal_point(scene_cfg.pallet.prim_path,scene_cfg.forklift.prim_path)
# (여기까지 수정함)
